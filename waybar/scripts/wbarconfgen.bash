...
[ -f "${scrDir}/globalcontrol.bash" ] && source "${scrDir}/globalcontrol.bash"
export waybar_dir="$HOME/dotfiles/waybar"

modules_dir="${waybar_dir}/modules"
conf_file="${waybar_dir}/config.jsonc"
conf_ctl="${waybar_dir}/config.ctl"

# Verify if the control file exists before proceeding
if [ ! -f "$conf_ctl" ]; then
    echo "Error: Control file not found at $conf_ctl"
    exit 1
fi
...
then
    ...
    do
        ...
        if [ "$flag" -eq 1 ] && [ "$1" == "n" ] ; then
            ...
            nextIndex=$(( (i - 1 + num_file) % num_files ))
            switch=1
            break;
        fi
    done
...
# Define hydeThemeDir if not set by globalcontrol
if [ -z "$hydeThemeDir" ]; then
    export hydeThemeDir="$HOME/dotfiles/hypr/themes"
fi

# Fallback in case the theme file doesn't exist
if [ ! -f "${hydeThemeDir}/hypr.theme" ]; then
    echo "Notice: hypr.theme not found, using default Icon Theme"
    export i_theme="Papirus"
else
    export i_theme="$(
