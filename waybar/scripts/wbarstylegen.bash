...
src_file="$HOME/dotfiles/hypr/themes/theme.conf"
backup_theme="$HOME/.config/hypr/themes/theme.conf"

if [ ! -f "$src_file" ] && [ -f "$backup_theme" ]; then
    src_file="$backup_theme"
fi
...
# Verify that core style variables are not empty
if [ -z "$b_height" ] || [ -z "$s_fontpx" ]; then
    echo "Error: Style calculations failed. Check your config.ctl and monitor resolution."
    exit 1
fi
...
if [ -f "$src_file" ]; then
    ...
    echo "Notice: $src_file not found, skipping corner rounding override."
fi
